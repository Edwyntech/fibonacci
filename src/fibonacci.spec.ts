...
describe('fibonacci series', () => {
    ...
    it('computes 4.3×10^208 for index 1 000', t => {
        const _1_000 = BigInt(EXPECTED_FIBONACCI_VALUES._1_000);
        assert.equal(fibonacci.of(1_000), _1_000);
    })

    it('computes 3.4×10^2089 for index 10 000', t => {
        const _10_000 = BigInt(EXPECTED_FIBONACCI_VALUES._10_000);
        assert.equal(fibonacci.of(10_000), _10_000);
    })

    it('computes 2.6×10^20898 for index 100 000', t => {
        const _100_000 = BigInt(EXPECTED_FIBONACCI_VALUES._100_000);
        assert.equal(fibonacci.of(100_000), _100_000);
    })

    it('computes 1.9×10^208987 for index 1 000 000', t => {
        const _1_000_000 = BigInt(EXPECTED_FIBONACCI_VALUES._1_000_000);
        assert.equal(fibonacci.of(1_000_000), _1_000_000);
    })
